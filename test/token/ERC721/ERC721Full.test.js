...
describe.only('ERC721Full', function () {
    ...
    context('with minted tokens', function () {
        ...
        describe.only('tokenByIndex', function () {
            it('returns all tokens', async function () {
                const tokensListed = await Promise.all(
                    [0, 1].map(i => this.contract.tokenByIndex(i)),
                );
                expect(tokensListed.map(t => t.toNumber())).to.have.members([this.firstTokenId,
                    this.secondTokenId]);
            });
      
            it('reverts if index is greater than supply', async function () {
                await expectRevert(
                    this.contract.tokenByIndex(2), 'ERC721Enumerable: global index out of bounds',
                );
            });
